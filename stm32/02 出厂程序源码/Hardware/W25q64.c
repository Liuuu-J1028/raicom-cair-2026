...
void SpiFlashWriteChar(char tmp,u32 WriteAddr) {
	...
	//Usart1_Send_Str((u8 *)"Byte has been written.\r\n");
} 
...
void SpiFlashWritePage(u8* pBuffer,u32 WriteAddr,u16 NumByteToWrite) {
	...
	if(NumByteToWrite>pageremain){
		...
		//Usart1_Send_Str((u8 *)"There are not enough writable bytes left in this page!\r\n");
	}
  ...
  for(i=0;i<NumByteToWrite;i++){ 
	...
	//Usart1_Send_Str((u8 *)" bytes have been written.\r\n");
} 
...
void SpiFlashWriteSector(u8* pBuffer,u32 WriteAddr,u16 NumByteToWrite) {
	...
	if(NumByteToWrite>sectorremain){
		...
		//Usart1_Send_Str((u8 *)"There are not enough writable bytes left in this sector!\r\n");
	}
...
void SpiFlashEraseSector(u32 Dst_Addr) {  
	...
	//Usart1_Send_Str((u8 *)"Sector has been erased.\r\n");
}
...
void SpiFlashEraseChip(void) {                                   
	...
	//Usart1_Send_Str((u8 *)"Chip has been erased.\r\n");
} 
