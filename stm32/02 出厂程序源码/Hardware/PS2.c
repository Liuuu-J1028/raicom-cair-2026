...
void psx_write_read(unsigned char *get_buf) {
	PS2_ATT(0);
	get_buf[0] = psx_transfer(START_CMD);       //无用数据
	get_buf[1] = psx_transfer(ASK_DAT_CMD);     //ID
	get_buf[2] = psx_transfer(get_buf[0]);      //准备完成标志
	get_buf[3] = psx_transfer(get_buf[0]);      
	get_buf[4] = psx_transfer(get_buf[0]);      //第四帧、第五帧为数字模式（按键）及模拟红灯模式下（按键及摇杆）的数据
	get_buf[5] = psx_transfer(get_buf[0]);
	get_buf[6] = psx_transfer(get_buf[0]);      //第六帧、第七帧为右摇杆的数据
	get_buf[7] = psx_transfer(get_buf[0]);
	get_buf[8] = psx_transfer(get_buf[0]);	    //第八帧、第九帧为左摇杆的数据
	PS2_ATT(1);
