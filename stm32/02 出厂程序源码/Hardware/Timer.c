...
void Servos_Inc_Handle(u8 index) {	
	int Aim_Temp;
	
	if(Servos_Doing[index].inc != 0) {
		
		Aim_Temp = Servos_Doing[index].aim;
		...
		if(abs_float(Aim_Temp - Servos_Doing[index].cur) <= abs_float(Servos_Doing[index].inc + Servos_Doing[index].inc)) {
			Servos_Doing[index].cur = Aim_Temp;
			Servos_Doing[index].inc = 0;
		} else {
			Servos_Doing[index].cur += Servos_Doing[index].inc;
		}
	}
}
...
void TIM2_IRQHandler(void) {
	...
	if (TIM_GetITStatus(TIM2, TIM_IT_Update) != RESET) //检查 TIM2 更新中断发生与否
	{
		...
		if(flag == 0) {
			TIM2->ARR = ((unsigned int)(Servos_Doing[Servos_Index1].cur));
			Servos_IO_Set(Servos_Index1, 1);	
			Servos_Inc_Handle(Servos_Index1);
		} else {
			temp = 2500 - (unsigned int)(Servos_Doing[Servos_Index1].cur);
			TIM2->ARR = temp;
			Servos_IO_Set(Servos_Index1, 0);
			Servos_Index1 ++;
		}
		flag = !flag;
	}
